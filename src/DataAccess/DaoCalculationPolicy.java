...
public class DaoCalculationPolicy implements Dao<String>{
    ...
    public List<String> get(List<String> keys) throws ParseException {
        ...
        CalculationpolicysRecord calculationpolicysRecord=create.selectFrom(CALCULATIONPOLICYS)
                .where(CALCULATIONPOLICYS.NAME.eq(key)).fetchOne();
        /** key noy found in table  **/
        if (calculationpolicysRecord == null || calculationpolicysRecord.size()==0){
            //return null;
            throw new ParseException("key noy found in table",0);
        }

        for (int i = 0; i <calculationpolicysRecord.size() ; i++) {
            if(calculationpolicysRecord.get(i) instanceof String){
                result.add(calculationpolicysRecord.get(i).toString());
            }
        }
        return result;
    }
    ...
    public List<List<String>> getAll(String collName, String filter) {
        ...
        if( collName==null && filter==null){
            ...
            Result<Record> result=create.select().from(CALCULATIONPOLICYS).fetch();
        ...
        String sql="SELECT * FROM calculationpolicys WHERE "+collName+"= '" + filter + "'";
        try {
    ...
    public void save(List<String> strings) throws SQLException {
        ...
        Result<Record> isExist = create.select().from(CALCULATIONPOLICYS)
                .where(CALCULATIONPOLICYS.NAME.eq(key)).fetch();

        if(isExist.size()== 0){ // key not exist
            /**add new row to DB **/
            try{
                create.insertInto(CALCULATIONPOLICYS,
                        CALCULATIONPOLICYS.NAME)
    ...
    public void update(List<String> keys, List<String> string) {
        ...
        if(calculationpolicysRecord != null) {

            /**update row in  DB **/
            calculationpolicysRecord.setName(string.get(0));
            try{
                calculationpolicysRecord.store();
    ...
    public void delete(List<String> strings) {
        ...
        create.delete(CALCULATIONPOLICYS)
                .where(CALCULATIONPOLICYS.NAME.eq(key))
                .execute();
