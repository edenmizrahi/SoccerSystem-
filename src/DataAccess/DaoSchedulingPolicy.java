...
import static DB.Tables.Tables.SCHEDULINGPOLICYS;

public class DaoSchedulingPolicy implements Dao<String> {
    @Override
    public List<String> get(List<String> keys) throws ParseException{
        ...
        SchedulingpolicysRecord schedulingpolicysRecord=create.selectFrom(SCHEDULINGPOLICYS)
                .where(SCHEDULINGPOLICYS.NAME.eq(key)).fetchOne();
        /** key noy found in table  **/
        if (schedulingpolicysRecord == null || schedulingpolicysRecord.size()==0){
            //return null;
            throw new ParseException("key noy found in table",0);
        }

        for (int i = 0; i <schedulingpolicysRecord.size() ; i++) {
            if(schedulingpolicysRecord.get(i) instanceof String){
                result.add(schedulingpolicysRecord.get(i).toString());
            }
        }
        return result;
    }
    ...
    public List<List<String>> getAll(String collName, String filter) {
        ...
        if( collName==null && filter==null){
            ...
            Result<Record> result=create.select().from(SCHEDULINGPOLICYS).fetch();
        ...
        int numOfCols=0;
        String sql="SELECT * FROM schedulingpolicys WHERE "+collName+"= '" + filter + "'";
        try {
    ...
    public void save(List<String> strings) throws SQLException {
        ...
        Result<Record> isExist = create.select().from(SCHEDULINGPOLICYS)
                .where(SCHEDULINGPOLICYS.NAME.eq(key)).fetch();

        if(isExist.size()== 0){ // key not exist
            /**add new row to DB **/
            try{
                create.insertInto(SCHEDULINGPOLICYS,
                        SCHEDULINGPOLICYS.NAME)
    ...
    public void update(List<String> keys, List<String> string) {
        ...
        if(schedulingpolicysRecord != null) {

            /**update row in  DB **/
            schedulingpolicysRecord.setName(string.get(0));
            try{
                schedulingpolicysRecord.store();
    ...
    public void delete(List<String> strings) {
        ...
        create.delete(SCHEDULINGPOLICYS)
                .where(SCHEDULINGPOLICYS.NAME.eq(key))
                .execute();
