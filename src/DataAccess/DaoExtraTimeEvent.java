...
import static DB.Tables.Tables.EVENTS_TIME;

/** table name : events_time**/
public class DaoExtraTimeEvent implements Dao<String>{
    @Override
    public List<String> get(List<String> keys) throws ParseException{
        ...
        EventsTimeRecord eventsTimeRecord=create.selectFrom(EVENTS_TIME)
                .where(EVENTS_TIME.EVENT_ID.eq(key)).fetchOne();
        /** key noy found in table  **/
        if (eventsTimeRecord == null || eventsTimeRecord.size()==0){
            throw new ParseException("key noy found in table",0);
        }
        for (int i = 0; i <eventsTimeRecord.size() ; i++) {
            result.add(eventsTimeRecord.get(i).toString());
        }
        return result;
    ...
    public List<List<String>> getAll(String collName, String filter) {
        ...
        if( collName==null && filter==null){
            ...
            Result<Record> result=create.select().from(EVENTS_TIME).fetch();
    ...
    public void save(List<String> strings) throws SQLException {
        ...
        Result<Record> isExist = create.select().from(EVENTS_TIME)
                .where(EVENTS_TIME.EVENT_ID.eq(key)).fetch();

        if(isExist.size()== 0){ // key not exist
            /**add new row to DB **/
            try{
                create.insertInto(EVENTS_TIME,
                        EVENTS_TIME.EVENT_ID, EVENTS_TIME.EXTRA_TIME)
                        .values(Integer.parseInt(strings.get(0)),Integer.parseInt( strings.get(1))).execute();
    ...
    public void update(List<String> keys, List<String> string) {
        ...
        if(eventsTimeRecord != null) {

            /**update row in  DB **/
            eventsTimeRecord.setExtraTime(Integer.parseInt(string.get(0)));
            try{
                eventsTimeRecord.store();
    ...
    public void delete(List<String> strings) {
        ...
        create.delete(EVENTS_TIME)
                .where(EVENTS_TIME.EVENT_ID.eq(key))
                .execute();
