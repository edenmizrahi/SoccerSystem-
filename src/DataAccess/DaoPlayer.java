...
import static DB.Tables.Tables.PLAYERS;
...
public class DaoPlayer implements Dao<String> {
    ...
    public List<String> get(List<String> keys) throws  ParseException{
        ...
        PlayersRecord playersRecord =create.selectFrom(PLAYERS)
                .where(PLAYERS.USERNAME.eq(Key)).fetchOne();
        /** key noy found in table  **/
        if (playersRecord == null || playersRecord.size()==0){
            //return null;
            throw new ParseException("key noy found in table",0);
        }

        for (int i = 0; i <playersRecord.size() ; i++) {
            if(playersRecord.get(i) instanceof String){
                result.add(playersRecord.get(i).toString());
            }
        }
    ...
    public List<List<String>> getAll(String collName, String filter) {
        ...
        if( collName==null && filter==null){
            ...
            Result<Record> result=create.select().from(PLAYERS).fetch();
        ...
        String sql="SELECT * FROM players WHERE "+collName+"= '" + filter + "'"; //!!!!!!!!!!!!!!!!!!!!!!!!!
        try {
    ...
    public void save(List<String> strings) throws SQLException {
        ...
        Result<Record> isExist = create.select().from(PLAYERS)
                .where(PLAYERS.USERNAME.eq(key)).fetch();
        ...
        if(isExist.size()== 0){ // key not exist
            ...
            try{
                ...
                create.insertInto(PLAYERS,
                        PLAYERS.USERNAME, PLAYERS.PLAYERTEAMNAME, PLAYERS.ROLEATFIELD)
    ...
    public void update(List<String> keys, List<String> string) {
        ...
        if(playersRecord != null) {
            ...
            /**update row in  DB **/
            playersRecord.setPlayerteamname(string.get(0));
            playersRecord.setRoleatfield(string.get(1));
            try{
                playersRecord.store();
