...
public class DaoApprovedTeamReq implements Dao<String> {
    ...
    public List<String> get(List<String> keys) throws ParseException {
        ...
        TeamOwnerApprovedTeamsRecord teamOwnerApprovedTeamsRecord =create.selectFrom(TEAM_OWNER_APPROVED_TEAMS)
                .where(TEAM_OWNER_APPROVED_TEAMS.TEAM_OWNER.eq(key1))
                .and(TEAM_OWNER_APPROVED_TEAMS.TEAM_NAME.eq(key2))
                .fetchOne();
    ...
    public List<List<String>> getAll(String collName, String filter) {
        ...
        if( collName==null && filter==null){
            ...
            Result<Record> result=create.select().from(TEAM_OWNER_APPROVED_TEAMS).fetch();
        ...
        int numOfCols=0;
        String sql="SELECT * FROM team_owner_approved_teams WHERE "+collName+"= '" + filter + "'"; //!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    public void save(List<String> strings) throws SQLException {
        ...
        Result<Record> isExist = create.select().from(TEAM_OWNER_APPROVED_TEAMS)
                .where(TEAM_OWNER_APPROVED_TEAMS.TEAM_OWNER.eq(key0))
        ...
        if(isExist.size()== 0){ // key not exist
            /**add new row to DB **/
            try{
                create.insertInto(TEAM_OWNER_APPROVED_TEAMS,
                        TEAM_OWNER_APPROVED_TEAMS.TEAM_OWNER, TEAM_OWNER_APPROVED_TEAMS.TEAM_NAME)
    ...
    public void update(List<String> keys, List<String> string) {
        ...
        if(teamOwnerApprovedTeamsRecord != null) {

            /**update row in  DB **/
            teamOwnerApprovedTeamsRecord.setTeamOwner(key0);;
            teamOwnerApprovedTeamsRecord.setTeamName(key1);
            try{
                teamOwnerApprovedTeamsRecord.store();
    ...
    public void delete(List<String> strings) {
        ...
        create.delete(TEAM_OWNER_APPROVED_TEAMS)
                .where(TEAM_OWNER_APPROVED_TEAMS.TEAM_OWNER.eq(key0))
                .and(TEAM_OWNER_APPROVED_TEAMS.TEAM_NAME.eq(key1))
                .execute();
